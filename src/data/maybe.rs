...
use super::{Associative, Foldable, Monoid, Type, compose::Compose};
...
impl Traversable for Maybe<()> {
    ...
    {
        funexp!(|f, maybe_a| match DataExpr::destructure(maybe_a) {
            Maybe::Nothing => F::pure().apply_value(Maybe::Nothing).eval(),
